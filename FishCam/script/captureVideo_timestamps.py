...
def captureVideo(outDir, iterFileName, videoSettings, flagname=''):
    ...
    curDir = os.getcwd()
    with open(os.path.join(curDir, iterFileName), 'r') as iterFile:
    ...
    if len(flagname) > 0:
        flagname = '_' + flagname
    videofilename = os.path.join(outDir, f"{iterNumber}{flagname}_{timeStampStr}_{videoSettings['resolution'][0]}x{videoSettings['resolution'][1]}_awb-{videoSettings['AWB']}.{videoSettings['format']}")
    ...
    try:
        ...
        start = datetime.now()
        while (datetime.now() - start).seconds < videoSettings['duration']:
            camera.annotate_text = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            camera.wait_recording(1)
        camera.stop_recording()
    except PiCameraValueError as e:
        logging.error(f"PiCameraValueError: {e}")
    finally:
        camera.close()

    # Updates iterator file
    with open(os.path.join(curDir, iterFileName), 'w') as iterFile:
        iterNumber += 1
        iterFile.write(str(iterNumber))

# Additional functions (isCameraOperational, captureVideo_loop, etc.) remain unchanged
